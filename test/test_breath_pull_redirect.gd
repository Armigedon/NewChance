...
func test_pull_during_windup_rotates_cone_aim() -> void:
	...
	var expected: Vector3 = Vector3.FORWARD.rotated(Vector3.UP, deg_to_rad(boss.CONE_REDIRECT_PER_PULL_DEG))
...
func test_two_pulls_compound() -> void:
	# Spec §4: cumulative redirect; two same-side pulls = 2 × CONE_REDIRECT_PER_PULL_DEG total.
	breath.trigger(1)
	breath._aim_dir = Vector3.FORWARD
	boss.apply_pull_toward(Vector3(2, 0, 1), 1.0)
	boss.apply_pull_toward(Vector3(2, 0, 1), 1.0)
	var aim: Vector3 = breath.current_aim()
	var expected: Vector3 = Vector3.FORWARD.rotated(Vector3.UP, deg_to_rad(2.0 * boss.CONE_REDIRECT_PER_PULL_DEG))
	assert_float(aim.x).is_equal_approx(expected.x, 0.01)
	assert_float(aim.z).is_equal_approx(expected.z, 0.01)
